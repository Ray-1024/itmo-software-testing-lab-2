...
public class CotTest extends BasePrecisionedFunctionTest {
    ...
    @Test
    void zero() {
        assertThrows(IllegalArgumentException.class, () -> cot.evaluate(ZERO.round(context), precision, context));
    }

    @Test
    void pi_2() {
        assertEqualsByPrecisionAndContext(ZERO.round(context), cot.evaluate(PI_2, precision, context));
    }

    @Test
    void pi() {
        assertThrows(IllegalArgumentException.class, () -> cot.evaluate(PI.round(context), precision, context));
    }

    @Test
    void pi3_2() {
        assertEqualsByPrecisionAndContext(ZERO.round(context), cot.evaluate(PI3_2.round(context), precision, context));
    }
